...
def azimuthalAverage(image, center=None, stddev=False, returnradii=False, return_nr=False, 
    ...
    if not psd_scale:
        ...
        maxwx = np.max(np.abs(x - center[0]))
        maxwy = np.max(np.abs(y - center[1]))
        minmaxw = min(maxwx, maxwy)
        r = np.hypot((x - center[0])/maxwx*minmaxw, (y - center[1])/maxwy*minmaxw)
