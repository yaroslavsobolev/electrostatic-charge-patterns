...
def PSD2(image, image2=None, oned=False, 
    ...
    if wavnum_scale:
        ...
        wx = numpy.concatenate([image.shape[0]/2 - numpy.arange(image.shape[0]/2,dtype='float'), numpy.arange(image.shape[0]/2,dtype='float') - 1]) / (image.shape[0]/2.)
        wy = numpy.concatenate([image.shape[1]/2 - numpy.arange(image.shape[1]/2,dtype='float'), numpy.arange(image.shape[1]/2,dtype='float') - 1]) / (image.shape[1]/2.)
        wx/=wx.max()
        wy/=wy.max()
        import matplotlib.pyplot as plt
        # wavnum = numpy.sqrt(numpy.outer(wx, numpy.ones(wx.shape)) ** 2 + numpy.outer(numpy.ones(wy.shape), wx) ** 2)
        wavnum = 1/numpy.sqrt(2)*numpy.sqrt( numpy.outer(wx,numpy.ones(wy.shape))**2 + numpy.outer(numpy.ones(wx.shape),wy)**2 )
        psd2 *= wavnum
...
def pspec(psd2, return_index=True, wavenumber=False, return_stddev=False, azbins=1, binsize=1.0, view=False, **kwargs):
    ...
    azbins,(freq,zz) = azimuthalAverageBins(psd2,azbins=azbins,interpnan=True,
                                            binsize=binsize, psd_scale=True, **kwargs)
    if len(zz) == 1: zz=zz[0]
    ...
    if return_index:
        ...
        return_vals = list(zz)
    if return_stddev:
        zzstd = azimuthalAverageBins(psd2,azbins=azbins,stddev=True,interpnan=True,
                                     binsize=binsize, psd_scale=True, **kwargs)
