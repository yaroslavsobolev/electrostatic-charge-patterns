...
frames_directory = 'experimental_data\\PDMS-PMMA_delamination_experiments\\data\\20191120_5cm_3in_72RH_ambRH21_eq30min_newPDMS5to1_PMMAtol_newwire_uniformspeeds_slowstart_0p6_E01\\video'
target_folder = 'experimental_data/PDMS-PMMA_delamination_experiments/data/20191120_5cm_3in_72RH_ambRH21_eq30min_newPDMS5to1_PMMAtol_newwire_uniformspeeds_slowstart_0p6_E01/'
...
frames_per_sec = 60
stamp_fall_video_frame = 340+322
c = 0.0033
testing = True
delta_t_for_graphs = 0.5
...
video_frames_at_discharge = [4+int(round(x*frames_per_sec)) for x in video_frames_at_discharge]
...
plt.axis('equal')
plt.xlim(0, 850)
to_exclude = list(range(0, 35, 1)) + list(range(56, 71, 1)) + list(range(100, 115, 1))
# to_exclude = []
for i in range(processed_stack_of_splines.shape[0]):
    if i in video_frames_at_discharge[2:]:
...
xs_here = np.arange(front_speed.shape[0])/frames_per_sec
ys_here = front_speed*frames_per_sec/pix_per_mm
mask_here = np.zeros_like(xs_here, dtype=np.bool)
mask_here[to_exclude] = 1
ys2_here = np.ma.masked_where(mask_here, ys_here)
plt.plot(delta_t_for_graphs + xs_here, ys2_here)
plt.ylim(2.8, np.max(front_speed*frames_per_sec/pix_per_mm)*1.3)
plt.xlim(0, 6)
...
ax7b.plot(time_for_graph - electrometer_minus_video_time_difference + delta_t_for_graphs, discharge_magnitude_graph, color='C1', linewidth=0.6, alpha=0.7)
video_frames_at_discharge = []
for step_loc in peaks[:]:
    time_here = t[steplen + delamination_start_index + step_loc]
    ax7b.axvline(x=time_here - electrometer_minus_video_time_difference + delta_t_for_graphs, color='grey', alpha=0.7, ymax=0.6)
