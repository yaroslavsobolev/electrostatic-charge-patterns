...
istart = 340 #int(round(57*30))+5+20
iend = 660  #2280
c = 0.0033
deriv_step = 2
testing = True
...
# #show one frame
# plt.imshow(vid.get_data(istart))
# plt.show()
# plt.imshow(vid.get_data(istart)[270:320, 1200:1245, :])
# plt.show()


# # find a time where the frame changes the most -- this will be the moment when the stamp falls off
# previous_frame = vid.get_data(1)
# indices = []
# diffs_of_frames = []
...
def stabilize_video(patch_coordinates, testing=False):
    ...
    for i, im in enumerate(vid):
        ...
        print('Frame={0}. Shift={1}'.format(i, np.linalg.norm(shift)))
    all_frames = np.stack(frames)
    return all_frames, shift_magnitudes

# frames, shift_magnitudes = stabilize_video([270, 320, 1200, 1245], testing=False)
...
# plt.show()
# roi = [142, 357, 385, 1192]
# plt.imshow(frames[70][roi[0]:roi[1], roi[2]:roi[3]])
# plt.show()
#
# nums = [10, 40]
# for num in nums:
...
def find_fronts_on_frames():
    list_of_dirs = ['\\delamination_fronts',
                    '\\delamination_fronts\\stripes',
                    '\\delamination_fronts\\diffs',
                    '\\delamination_fronts\\fronts']
    for dir in list_of_dirs:
        if not os.path.exists(frames_directory + dir):
            os.mkdir(frames_directory + dir)
            print('Created dir: {0}'.format(dir))
