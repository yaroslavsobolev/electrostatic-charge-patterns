...
for i, front_curve in enumerate(front_curves[:-10]):
    ...
    if len(ys) > 5:
        ...
        xs2 = splev(ys2, tck)
        plt.plot(xs2, ys2, color='red', alpha=0.5)
...
for i in range(processed_stack_of_splines.shape[0]):
    plt.plot(processed_stack_of_splines[i,:], ys_for_splines, color='red', alpha=0.5)
...
for step_loc in peaks:
    ...
    ax7b.axvline(x=time_here - electrometer_minus_video_time_difference, color='grey', alpha=0.7)
